...
public class JsonParser
{
	...
	private static void readResponseVO(ResponseVO vo, JsonReader reader) throws IOException
	{
		...
		while (reader.hasNext())
		{
			...
			if (name.equals("code")
					&& (JsonToken.NUMBER.equals(reader.peek()) || JsonToken.STRING.equals(reader.peek())))
				try
				{
					vo.setCode(reader.nextInt());
				} catch (Exception e)
				{
					vo.setCode(ResponseVO.RESPONSE_CODE_FAIL);
				}
	 ...
	 *            存储报文状态信息(需进行实例化)
	 * @param ref
	 *            ref内容，如果ref==null或"" 则读取第一个entity开使的内容
	 * @return
	 */
	...
	private static Boolean IsProp(JsonToken jt)
	{
		return (!JsonToken.NULL.equals(jt))
				&& (JsonToken.BOOLEAN.equals(jt) || JsonToken.NUMBER.equals(jt) || JsonToken.STRING.equals(jt));
	}

	private static Boolean IsList(JsonToken jt)
	{
		return (!JsonToken.NULL.equals(jt)) && JsonToken.BEGIN_ARRAY.equals(jt);
	}

	private static Boolean IsEntity(JsonToken jt)
	{
		return (!JsonToken.NULL.equals(jt)) && (JsonToken.BEGIN_OBJECT.equals(jt));
	}
	 ...
	 *            存储报文状态信息(需进行实例化)
	 * @param ref
	 *            ref内容，如果ref==null或"" 则读取第一个list开使的内容
	 * @return
	 */
	...
	private static Object convertToEntity(JsonReader reader, String classStr)
	{
		...
		try
		{
			...
			while (reader.hasNext())
			{
				...
				// else
				// if(name.startsWith(TAG_ENTITY+"_")&&JsonToken.BEGIN_OBJECT.equals(reader.peek()))//entity_class_ref
				...
				try
				{
					Method method = obj.getClass().getMethod(
							"set" + propName.substring(0, 1).toUpperCase() + propName.substring(1), paraType);
					method.invoke(obj, nodeObj);
				} catch (Exception e)
				{
					Log.e("JsonParser.convertToEntity() " + "set" + propName.substring(0, 1).toUpperCase()
							+ propName.substring(1), e.toString());
					continue;
				}
			}
			reader.endObject();
