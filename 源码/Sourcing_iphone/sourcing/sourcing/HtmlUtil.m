...
+ (NSString *) getHtmlHead
{
    ...
    return [NSString stringWithFormat:@"<html><head><meta http-equiv=\"Content-Type\" content=\"text/html;charset=utf-8\">%@</head><body style=\"background-color:#EEEEEE\">", autoloadimgscript];
}
...
+ (NSString *) getHtmlContext:(NSString *)newscontent
{
    ...
    while (range.location != NSNotFound) {
        ...
        if(rangesrc.location != NSNotFound)
        {
            ...
            {
                if([[src lowercaseString] hasPrefix:[[DataInterfaceUtil getDataInterface:@"serverstartname"] lowercaseString]])
                {
                    src = [NSString stringWithFormat:@"%@%@",[DataInterfaceUtil getDataInterface:@"publicurl"],src];
                }
                [result appendFormat:@" <a href=\"%@\"> ",src];
                [result appendFormat:@"<img src=\"%@\" %@ /></a>",src,CSTR_IMG_INSERT];
                
            }
        }
        
        
//        NSRange range2 = [tmp rangeOfString:CSTR_IMG_REMOVE_START];
//        NSRange rangesrc = [tmp rangeOfString:CSTR_IMG_SRC];
//        NSString *src=@"";
//        if(range2.location != NSNotFound)
//        {
//            if(rangesrc.location != NSNotFound)
//            {
//                rangesrc.location = rangesrc.location+rangesrc.length;
//                rangesrc.length = range2.location-1;
//                src = [tmp substringWithRange:rangesrc];
//            }
//            if(![StringUtil isEmptyStr:src])
//                [result appendFormat:@" <a href=%@> ",src];
//            [result appendString:[tmp substringToIndex:range2.location]];
//            tmp = [tmp substringFromIndex:range2.location];
//            range2 = [tmp rangeOfString:CSTR_IMG_REMOVE_END];
//            if(range2.location!=NSNotFound&&range2.location!=tmp.length)
//            {
//                [result appendString:[tmp substringFromIndex:range2.location+1]];
//            }
//        }
//        else
//        {
//            if(rangesrc.location != NSNotFound)
//            {
//                src = [tmp substringFromIndex:rangesrc.location+4];
//            }
//            if(![StringUtil isEmptyStr:src])
//                [result appendFormat:@" <a href=%@> ",src];
//            [result appendString:tmp];
//        }
//        [result appendString:CSTR_IMG_INSERT];
//        
//        if(![StringUtil isEmptyStr:src])
